...
int main(int argc, char** argv) {
	...
	if (argc < 4) {
		printf("usage:\n\t %s <img_source> <img _dest> <aling_img_dest\n", argv[0]);
		exit(1);
	}
    ...
    iftImage* sob2 = iftAbs(sob);
    iftDestroyImage(&sauv);
    iftDestroyImage(&sob);

    iftImage *filter_img = iftOpen(sob2, A);
    iftDestroyImage(&sob2);

	iftImage *candidate = computeDenserRegions("", &v, filter_img, img, "points.txt");
	...
	iftImage* labelTrue = drawLabelGen(img, center, 20, 10);

	iftImage* sum_img = iftAdd(img, labelTrue);
	iftWriteImageP2(sum_img, "sum_img.pgm");
	iftDestroyImage(&sum_img);

	iftImage* labelFalse = drawLabelGen(img, center, 85, 36);

	iftImage *label = iftPlateWatershed(img, labelTrue, labelFalse);
	...
	iftImage *cimg = iftCorrectImage(img,src,dst,200,60);
	iftWriteImageP5(cimg,argv[3]);
	iftDestroyImage(&cimg);

	sum_img = iftAdd(img, watershed_img);
	iftWriteImageP2(sum_img, argv[2]);
	iftDestroyImage(&sum_img);
