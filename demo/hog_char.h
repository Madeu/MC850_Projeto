...
iftImage *firstStep_normalize_v2(iftImage *orig) {
	iftAdjRel *adj = iftCircular(HOG_r_normalize);
	iftImage* img_normalized = iftCreateImage(orig->xsize, orig->ysize,
			orig->zsize);
...
void secondStep_gradient_v2(iftImage *img, iftImage **g_mag,
		iftImage **g_orient) {
	iftAdjRel *adj = iftCircular(HOG_r_gradient);
...
void celulas_adjacentes(int i, int j, int *celula1, int *celula2, int *celula3,
		int *celula4) {
...
iftFeatures *thirdStep_histogram(iftImage *g_mag, iftImage *g_orient, int x0,
		int y0) {
	...
	//Concatena
	int numBlocos = (HOG_N1 / HOG_N2 - HOG_N3 + 1)
			* (HOG_M1 / HOG_M2 - HOG_M3 + 1);
	iftDataSet *histogramasConcatNormalizados = iftCreateDataSet(numBlocos,
			HOG_N3 * HOG_M3 * numBins);
	...
	for (int k = 0; k < numBlocos; k++) {
		...
		for (int i = 0; i < HOG_N3; i++) {
			int celula = (k / numBlocosHorizontal) * numCelulasHorizontal
					+ i * numCelulasHorizontal
					+ k % (numCelulasHorizontal - HOG_N3 + 1);
			for (int j = 0; j < HOG_M3; j++) {
				//printf("Bloco %d, feat %d, celula %d \n", k, i*HOG_N3*numBins+j*numBins, celula+j);
				for (int b = 0; b < numBins; b++) {
					histogramasConcatNormalizados->sample[k].feat[i * HOG_N3
							* numBins + j * numBins + b] = histogramas[celula
							+ j]->val[b];
	...
	for (int k = 0; k < numBlocos; k++) {
		for (int i = 0; i < HOG_N3 * HOG_M3 * numBins; i++) {
			hog->val[k * histogramasConcatNormalizados->nfeats + i] =
					histogramasConcatNormalizados->sample[k].feat[i]
							/ (sqrt(soma[k]) + 0.000001);
		}
	}
	free(soma);
...
iftVoxel iftCreateBoundingBox2D(iftImage *label, int val, iftImage *img_mag,
		iftImage *img_orient, iftImage **out_mag, iftImage **out_orient) {
	...
	iftImage *bb_mag = iftCreateImage((maxX - minX + 1), (maxY - minY + 1),
			img_mag->zsize);
	iftImage *bb_orient = iftCreateImage((maxX - minX + 1), (maxY - minY + 1),
			img_mag->zsize);
